![Work Report](../../image/baner/Untitled-1.jpg)
---
<h3 align="right"><b>فصل دوم 🔹</b></h3>
<h3 align="right"><b>قسمت: سوم 🎬</b></h3>
<h3 align="right"><b>۱۴۰۴/۰۹/۱۶ 📆</b></h3>


<h3 align="right"><b>"Dom navigation"</b></h3>

###### Home
> [توضیحات](#1) - [چیزهایی که یاد گرفتم](#2) - [سوالات و مشکلات](#3) - [کد ها و خروجی ها](#4)

---
###### 1
### 🔍 توضیحات
DOM Navigation مجموعه‌ای از ابزارها و ویژگی‌هاست که به ما اجازه می‌دهد داخل ساختار درختی DOM حرکت کنیم و به هر بخش از صفحه دسترسی داشته باشیم. علاوه بر ویژگی‌هایی مثل **childNodes**، **parentNode** و **nextSibling/previousSibling** که برای پیمایش بین گره‌ها استفاده می‌شن، وقتی با جدول‌ها کار می‌کنیم مجموعه‌ای از ویژگی‌های اختصاصی مثل **table.rows** برای دسترسی به ردیف‌ها، **tr.cells** برای دسترسی به سلول‌های هر ردیف، و **rowIndex** یا **sectionRowIndex** برای فهمیدن موقعیت هر ردیف در جدول یا بخش مربوطه اهمیت پیدا می‌کنه. همچنین المنت‌های ساختاری مثل **table.caption**، **tHead**، **tFoot** و **tBodies** اجازه می‌دن بخش‌های مختلف جدول رو جداگانه مدیریت کنیم. این قابلیت‌ها باعث می‌شن بتونیم ساختار صفحه رو دقیق درک کنیم و کنترل کاملی روی محتوا و رفتار عناصر مختلف داشته باشیم.
> [Home](#home)
---
###### 2
## 🧠 چیزهایی که یاد گرفتم

- مفهوم DOM Navigation و اینکه ساختار صفحه مثل یه درخته
- دسترسی به فرزندان یک المنت با `childNodes`
- دسترسی به والد یک المنت با `parentNode`
- حرکت بین گره‌های هم‌سطح با `nextSibling` و `previousSibling`
- پیدا کردن تعداد فرزندان یک المنت
- گرفتن یک المنت خاص از بین فرزندان (مثل `childNodes[1]`)

### بخش مربوط به جدول‌ها (Table Navigation)
- دسترسی به ردیف‌های جدول با `table.rows`
- دسترسی به سلول‌های هر ردیف با `tr.cells`
- تشخیص شماره‌ی ردیف در کل جدول با `tr.rowIndex`
- تشخیص شماره‌ی ردیف داخل بخش خودش (مثلاً داخل `tbody`) با `tr.sectionRowIndex`
- دسترسی به قسمت‌های ساختاری جدول:
  - `table.caption`
  - `table.tHead`
  - `table.tFoot`
  - `table.tBodies`

> [Home](#home)

###### 3
## سوالات و مشکلات⁉️


> [Home](#home)
---
###### 4
## 🧪 تست‌ها و خروجی‌ها
- ![Work Report](../E03/code.png)
> [Home](#home)